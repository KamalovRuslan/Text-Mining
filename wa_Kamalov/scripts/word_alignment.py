...
def get_posterior_distribution_for_trg_token(trg_index, src_tokens, trg_tokens,
    ...
    trg = trg_tokens[trg_index]
    #trg_tag = trg_tags[trg_index]
    joint_probs = [prior_model.get_prior_prob(i, trg_index, len(src_tokens), len(trg_tokens)) *
                translation_model.get_conditional_prob(src_tokens[i], trg) for i in range(len(src_tokens))]
    marginal_prob = sum(joint_probs)
    posterior_probs = list(map(lambda t : t / marginal_prob, joint_probs))
...
if __name__ == "__main__":
    ...
    #src_tag_corpus = utils.read_all_tags(sys.argv[3])
    #trg_tag_corpus = utils.read_all_tags(sys.argv[4])
