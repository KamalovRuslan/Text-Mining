import codecs

def output_alignments_per_test_set(alignments, output_prefix, test_sets={"blinds" : (0, 171),
                                                                         "dev" : (171, 341),
                                                                         "test" : (341, 511)}):
    "Outputs word alignments for test sets: this assumes corpus was from start of .all file"
    for test, (start, end) in test_sets.items():
        if len(alignments) < end:
            print "skipping test set %s." % test
            continue
        output = open(output_prefix + "." + test + ".wa", "w")
        for alignment in alignments[start:end]:
            output.write(" ".join(["%d-%d-*" % (src_index, trg_index)
                                   for src_index, trg_index in alignment]) + "\n")

def read_all_tokens(path):
    return [[tok[:4] for tok in line.strip().split()] for line in codecs.open(path, 'r', 'utf8')]

def read_all_tags(path):
    return [[tag for tag in line.strip().split()] for line in codecs.open(path, 'r', 'utf8')]

def read_all_lemmas(path):
    return [[lemma for lemma in line.strip().split()] for line in codecs.open(path, 'r', 'utf8')]
